...
#parametros requeridos
$usuarioS = $_POST['usuarioSolicitante'];
...
if (!$_POST) {
    ...
    $folio = $_POST["_folioc"];
    $nocomprobantes = @$_POST["Nocomprobantes"];

}
#Recepcion de datos $_POST

$solicitud->set('id', $folio);
$solicitud->set('nocomprobantes', $nocomprobantes);
$solicitud->set('estatus', "pagado");

$answer = $solicitud->registraComprobantes();

if ($answer) {
    $nombre_archivo ="";$nombres_archivos="";
    for($i=0;$i<count($_FILES["comprobantes"]["name"]);$i++){
        /* Lectura del archivo */
        $nombre_archivo = $_FILES['comprobantes']['name'][$i];
        $tipo_archivo   = $_FILES['comprobantes']['type'][$i];
        $tamano_archivo = $_FILES['comprobantes']['size'][$i];
        $tmp_archivo    = $_FILES['comprobantes']['tmp_name'][$i];
        ...
        if ($nombre_archivo != "" and !empty($folio)) {
            ...
            $num_compra=$folio;
            if($tamano_archivo!=0){
                $ruta_pancarta= str_replace(DS, "/", ROOT . 'Pagos/Comprobantes/'.$num_compra);
                $archivador= str_replace(DS, "/", ROOT . 'Pagos/Comprobantes/'.$num_compra);
                $dir_logo=$archivador."/".$nombre_archivo;
                
                if (file_exists(str_replace(DS, "/", ROOT . 'Pagos/Comprobantes/'.$num_compra))) {

                } else {
                    mkdir(str_replace(DS, "/", ROOT . 'Pagos/Comprobantes/'.$num_compra),0700);    
                }
                ...
                if(!copy($dir_logo,$archivador."/".$nombre_archivo)){
                    logs($folio,$usuarioS,'Carga Comprobante de Pago: '.$nombre_archivo); 
