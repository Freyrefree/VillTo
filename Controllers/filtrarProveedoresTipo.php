...
$proveedor->set("tipo",$tipo_proveedor);
if($tipo_proveedor != "todos"){
    $answer  = $proveedor->filtraProveedorTipo();
}else{
    $answer  = $proveedor->filtraProveedor();
}
if($tipo_proveedor == "nuevos"){
    $answer  = $proveedor->nuevosProveedores();
}
...
while($row = mysqli_fetch_array($answer)){
  ...
  if($row['activo'] == "re") $indica = '<i class="fa fa-certificate" style="color:#BA4A00" aria-hidden="true"></i> ';

    $data[] = array(
    	
      'id'    => $row['id'],
      'numproveedor' => $indica.utf8_encode($row['numproveedor']),
