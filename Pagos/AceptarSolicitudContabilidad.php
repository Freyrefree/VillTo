...
#parametros requeridos
$usuarioS = $_GET['nombreContabilidadCorreo'];
...
$row = mysqli_fetch_array($answer);
    ...
    $estatus = $row["estatus"];
    if(($estatus != "Aceptado") && (($estatus == "Aceptado Contabilidad") || ($estatus == "Rechazado Contabilidad") || ($estatus == "pendiente") || ($estatus == "Aceptado") || ($estatus == "Rechazado") || ($estatus == "Pago Programado") || ($estatus == "Rechazado Tesoreria"))){
        echo '<body>
        <div class="row">
            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-block titulo bg-primary">
                        <h5 style="float:left">Solicitudes / Pagos / Contabilidad</h5>
...
############## CORREO y nombre para notificar a  TESORERÍA ###############
$usuarioOBJ->usuarioTesoreria();
$receptor = $usuarioOBJ->get('correoTesoreria');
$nombre = utf8_encode($usuarioOBJ->get('nombre'))." ".utf8_encode($usuarioOBJ->get('app'))." ".utf8_encode($usuarioOBJ->get('apm'));

$asunto = "Aceptación Pago | Contabilidad | Folio: ".$idSolicitud." | Solicitante: ".$usuario.".";
...
$nombreTesoCorreo    =  utf8_encode($usuarioOBJ->get('nombre'))." ".utf8_encode($usuarioOBJ->get('app'))." ".utf8_encode($usuarioOBJ->get('apm'));
...
if($importancia == "si"){
    ...
    $linkAutoriza = URL . "Pagos/AceptarSolicitudTesoreria.php?id=".$idSolicitud."&nombreTesoCorreo=".$nombreTesoCorreo;
    $linkRechaza =  URL . "Pagos/PreRechazoTesoreria.php?id=".$idSolicitud."&nombreTesoCorreo=".$nombreTesoCorreo;
...
$solicitud->set('estatus', "Aceptado Contabilidad");


if($txt == ""){
    $res=correo($asunto,$receptor,$nombre,$html,$mailcc,$namecc);
    if($res){
        $answer2  = $solicitud->actualizaEstatus();
        if($answer2){
            $envio = true;
            logs($idSolicitud,$usuarioS,'Solicitud Aceptada Contabilidad');
        }
    }

}else{
    $res = correoTXT($asunto,$receptor,$nombre,$html,$mailcc,$namecc,$txt,$rutatxt);
    if($res){
        $answer2  = $solicitud->actualizaEstatus();
        if($answer2){
            $envio = true;
            ...
            <div class="card">
                ...
                <div class="card-block">
                    <div class="row">
                        <div class='col-md-12'>
                            <?php if($envio == true){ ?>
                            <h5><i class="fa fa-check-circle-o fa-1x" aria-hidden="true"></i> La solicitud de pago con el <strong>Folio: <?= $idSolicitud; ?> </strong> ha sido aceptado por Contabilidad</h5>
                            <br><br>
...
//*****************************FUNCIÓN Correo Con Archivos (txt)*******************************************/}
function correoTXT($asunto,$receptor,$nombre,$html,$mailcc,$namecc,$txt,$rutatxt)
{
    require_once('../lib/swiftmailer/swift_required.php');
    $retorno = true;
    ...
    try {
            ...
            ->setCc(array($mailcc =>$namecc))
            ->setBody($html, 'text/html') //body html
            ->attach(Swift_Attachment::fromPath($rutatxt)->setFilename($txt));
            if ($mailer->send($message)){
                //echo 1;
                $retorno = true;
            }else{
